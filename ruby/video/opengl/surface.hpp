inline auto OpenGLSurface::size(u32 w, u32 h) -> void {
...
inline auto OpenGLSurface::release() -> void {
  if(texture) { glDeleteTextures(1, &texture); texture = 0; }
  if(framebuffer) { glDeleteFramebuffers(1, &framebuffer); framebuffer = 0; }
  width = 0, height = 0;
}

inline auto OpenGLSurface::render(u32 sourceWidth, u32 sourceHeight, u32 targetX, u32 targetY, u32 targetWidth, u32 targetHeight, GLuint targetFramebuffer) -> void {
  glBindTexture(GL_TEXTURE_2D, texture);
