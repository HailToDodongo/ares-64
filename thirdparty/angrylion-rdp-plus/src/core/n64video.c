...
// Rendering metrics. Single-threaded increments only (config.parallel == false);
// enabling the worker pool would require these to be atomic.
static struct n64video_metrics metrics;

#ifdef N64VIDEO_METRICS
// Framebuffer access heatmap: one saturating counter per 16-bit RDRAM word, for reads
// and writes. Parallel-safe without atomics because workers partition the framebuffer by
// scanline, so no two workers ever touch the same word.
#define FB_HEATMAP_ENTRIES (RDRAM_MAX_SIZE >> 1)
static uint8_t fb_heatmap_writes[FB_HEATMAP_ENTRIES];
static uint8_t fb_heatmap_reads[FB_HEATMAP_ENTRIES];

static STRICTINLINE void
fb_heatmap_bump(uint8_t *buf, uint32_t word_idx)
{
    word_idx &= FB_HEATMAP_ENTRIES - 1;
    if (buf[word_idx] < 0xff) buf[word_idx]++;
}
#endif
...
{
    // cmd_count is accumulated on the main thread (in n64video_process_list)
    *out = metrics;
    out->pixels_drawn = 0;
    out->pixels_filled = 0;
#ifdef N64VIDEO_METRICS
    // pixel counters live per-worker to avoid cross-thread sharing; sum them here
    uint32_t n = config.parallel ? parallel_num_workers() : 1;
    for (uint32_t i = 0; i < n; i++) {
        out->pixels_drawn += state[i].metrics_pixels_drawn;
        out->pixels_filled += state[i].metrics_pixels_filled;
    }
#endif
}
...
n64video_metrics_reset(void)
{
    ...
    uint32_t n = config.parallel ? parallel_num_workers() : 1;
    for (uint32_t i = 0; i < n; i++) {
        state[i].metrics_pixels_drawn = 0;
        state[i].metrics_pixels_filled = 0;
    }
#endif
}
...
{
#ifdef N64VIDEO_METRICS
    out->writes = fb_heatmap_writes;
    out->reads = fb_heatmap_reads;
    out->entries = FB_HEATMAP_ENTRIES;
#else
    out->writes = NULL;
    out->reads = NULL;
    out->entries = 0;
...
{
#ifdef N64VIDEO_METRICS
    memset(fb_heatmap_writes, 0, sizeof(fb_heatmap_writes));
    memset(fb_heatmap_reads, 0, sizeof(fb_heatmap_reads));
#endif
}
...
#ifdef N64VIDEO_METRICS
            metrics.cmd_count[rdp_cmd_id & 0x3f]++;
#endif
...
{
    // rdram_hidden lives in the rdram.c module included into this translation unit.
    *buf = rdram_hidden;
    *entries = RDRAM_MAX_SIZE / 2;
}
...
n64video_tmem_snapshot(struct n64video_tmem_snapshot *out)
{
    ...
    out->tmem = state[0].tmem;
    for (int i = 0; i < N64VIDEO_NUM_TILES; i++) {
        out->tiles[i].format  = (uint8_t)state[0].tile[i].format;
        out->tiles[i].size    = (uint8_t)state[0].tile[i].size;
        out->tiles[i].tmem    = (uint16_t)state[0].tile[i].tmem;
        out->tiles[i].line    = (uint16_t)state[0].tile[i].line;
        out->tiles[i].palette = (uint16_t)state[0].tile[i].palette;
        out->tiles[i].sl      = state[0].tile[i].sl;
        out->tiles[i].tl      = state[0].tile[i].tl;
        out->tiles[i].sh      = state[0].tile[i].sh;
        out->tiles[i].th      = state[0].tile[i].th;
        out->tiles[i].clamp_s  = (uint8_t)state[0].tile[i].f.clampens;
        out->tiles[i].clamp_t  = (uint8_t)state[0].tile[i].f.clampent;
        out->tiles[i].mirror_s = (uint8_t)(state[0].tile[i].ct & 1);
        out->tiles[i].mirror_t = (uint8_t)(state[0].tile[i].mt & 1);
        out->tiles[i].mask_s   = (uint8_t)state[0].tile[i].mask_s;
        out->tiles[i].mask_t   = (uint8_t)state[0].tile[i].mask_t;
        out->tiles[i].shift_s  = (uint8_t)state[0].tile[i].shift_s;
        out->tiles[i].shift_t  = (uint8_t)state[0].tile[i].shift_t;
    }
}
                            ...
                            uint32_t x0, uint32_t y0, uint32_t w, uint32_t h)
{
    for (uint32_t y = 0; y < h; y++) {
        for (uint32_t x = 0; x < w; x++) {
            struct color c;
            fetch_texel_decode(&state[0], &c, (int)(x0 + x), (int)(y0 + y), tilenum);
            #define CL(v) ((uint32_t)((v) < 0 ? 0 : (v) > 255 ? 255 : (v)))
            dst[y * w + x] = (CL(c.a) << 24) | (CL(c.b) << 16) | (CL(c.g) << 8) | CL(c.r);
            #undef CL
        }
    }
}
